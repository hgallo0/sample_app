...
- Both exporters need the `cloudtrace.googleapis.com` API enabled and `roles/cloudtrace.agent` on the runtime service account - this is now committed, permanent infra (`main.tf`'s `services` list + `google_project_iam_member.compute_sa_cloudtrace_agent`), not something to re-provision each rehearsal. Don't re-discover or re-flag it; just confirm with a quick `gcloud services list`/`gcloud projects get-iam-policy` read that it's actually present in this environment before assuming the exporter has what it needs. If it's genuinely missing (e.g. a from-scratch environment that never ran `tofu apply` on current `main.tf`), that's a real gap worth flagging - a Tofu change (`google_project_service` in the `services` list, an IAM binding alongside the existing Cloud SQL/Redis ones), not something to grant via `gcloud`.
